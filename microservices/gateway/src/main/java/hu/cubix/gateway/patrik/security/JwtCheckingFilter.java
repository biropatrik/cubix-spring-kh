package hu.cubix.gateway.patrik.security;
...
public class JwtCheckingFilter implements GlobalFilter {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        if (orderPathPattern.matches(PathContainer.parsePath(originalUri.toString()).subPath(4)) &&
                ObjectUtils.isEmpty(authHeaders)) {
            return send401Response(exchange);
        } else if (catalogPathPattern.matches(PathContainer.parsePath(originalUri.toString()).subPath(4)) &&
                ObjectUtils.isEmpty(authHeaders) && !method.equals(HttpMethod.GET)) {
            return send401Response(exchange);
        } else if (!ObjectUtils.isEmpty(authHeaders)) {
